...
'''
Example:
python onimagestunningModify.py --target '/media/math-clinic/FastData/Data/n2V/Varun Ramani/2020-06-30/training/speed6' --baseDir '/media/math-clinic/FastData/Data/n2V/Varun Ramani/2020-06-30/training/model6' --dims XY --stack n --train y

os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID";  
os.environ["CUDA_VISIBLE_DEVICES"] = "1" # The GPU id to use, usually either "0", "1". "2", "3";

# To implement get number of GPU available
...
parser.add_argument('--baseDir', metavar='baseDir', type=str, default = 'models',
                help='directory path to store the trained models and the configurations.') # Not working properly!
...
def use_selected_card(gpu=args.gpu):
	if gpu==0:
...
def create_output_directory(output_path):
    if output_path is None:
        output_path = os.path.join(os.path.dirname(args.target),'denoised_images_'+str(os.path.basename(os.path.normpath(args.target)))+'_'+str(datetime.datetime.now()).replace(' ','_').replace('.','p').replace(':','T'))
